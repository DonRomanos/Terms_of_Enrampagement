...
int main(void)
{
    auto window = []{
        if (auto init_result = glfwInit(); !init_result) {
            std::cerr << "Initialization of GLFW failed with code: " << init_result << ", terminating." << std::endl;
            std::terminate();
      }
    ...
    auto glfw_input_provider = std::make_unique<input::GlfwInputProvider>(window);
